...
# Return if commands not found
command -v dmenu_run >/dev/null || exit 1
command -v wpg >/dev/null || exit 1

# Get Colors
colors="$(sed --quiet --regexp-extended 's/^\*\.((back|fore)ground|color(4|15))/p' "${HOME}/.config/wpg/formats/colors.Xresources" | \
	awk '/back/ {ngb = $NF} /fore/ {nfg = $NF} /r4/ {sbg = $NF} /r15/ {sfg = $NF} END {print nbg.nfg,sbg,sfg}')"
